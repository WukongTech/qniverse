...
class SVGP_Layer(Layer):
    def __init__(self, kern, Z, num_outputs, mean_function,
        ...
        self.q_mu = Parameter(q_mu, dtype=np.float32)
        ...
        self.q_sqrt = Parameter(q_sqrt, transform=transform, dtype=np.float32)
        ...
        if not self.white:  # initialize to prior
            ...
            Lu = np.linalg.cholesky(Ku + np.eye(Z.shape[0], dtype=np.float32)*settings.jitter)
